...
public class CanvasPanelHandler extends PanelHandler
{
	...
	int				boundShift	= 10;
	int highLightPort 			= 20210514;
	JPanel selectedObj			= null;
	...
	void selectByClick(MouseEvent e)
	{
		...
		for (int i = 0; i < members.size(); i ++)
		{
			if (isInside(members.elementAt(i), e.getPoint()) == true
					&& isSelect == false)
			{
				selectedObj = members.elementAt(i);
				switch (core.isFuncComponent(members.elementAt(i)))
				{
					case 0:
						...
						isSelect = true;
						// which lines should be highlighted
						// get port by mouse location() compare to the ports
						// all lines connected to this port should set isHighlight = true
						highLightPort = ((BasicClass) members.elementAt(i)).getSelectedPort(e);
						for (int j = 0; j < contextPanel.getComponentCount(); j++) {
//							System.out.println("components = " + contextPanel.getComponentCount());
//							System.out.println("just checking the lines" + core.isLine(members.elementAt(j)));
//							System.out.println("just checking the funcs" + core.isFuncComponent(members.elementAt(j)));
//							System.out.println("Port???: " + highLightPort);
							switch (core.isLine(contextPanel.getComponent(j))) {
								case 0:
									((AssociationLine) contextPanel.getComponent(j)).setHighlight(selectedObj, highLightPort);
									// System.out.println("ass line found!");
									break;
								case 1:
									((CompositionLine) contextPanel.getComponent(j)).setHighlight(selectedObj, highLightPort);
									break;
								case 2:
									((GeneralizationLine) contextPanel.getComponent(j)).setHighlight(selectedObj, highLightPort);
									break;
								case 3:
									((DependencyLine) contextPanel.getComponent(j)).setHighlight(selectedObj, highLightPort);
									break;
								default:
									break;	
							}
					...
					case 1:
						...
						// get port by mouse location() compare to the ports
						// all lines connected to this port should set isHighlight = true
						highLightPort = ((UseCase) members.elementAt(i)).getSelectedPort(e);
						for (int j = 0; j < contextPanel.getComponentCount(); j++) {
//							System.out.println("components = " + contextPanel.getComponentCount());
//							System.out.println("just checking the lines" + core.isLine(members.elementAt(j)));
//							System.out.println("just checking the funcs" + core.isFuncComponent(members.elementAt(j)));
...
//	void getSelectedPort(MouseEvent e) {
//		Point port = new Point(e.getPoint());
//		System.out.println("port x = " + port.x);
//		System.out.println("port y = " + port.y);
//	}
	...
	void resetHighlightAllType() {
		selectedObj = null;
		System.out.println("reset high light");
		for (int j = 0; j < contextPanel.getComponentCount(); j ++) {
			switch (core.isLine(contextPanel.getComponent(j))) {
			case 0:
				((AssociationLine) contextPanel.getComponent(j)).setHighlight(selectedObj, highLightPort);
